...
FOOT = '''
    ...
    <footer>
      Prepared from supplied D1, D9, D10, D11, D8, D27, D30, upagraha, Vimshottari, Shadbala, Bhava Bala, Ashtakavarga, Reduced Ashtakavarga, Shodhya Pinda and transit data. All divisional charts, dasha boundaries and strength tables were independently recomputed from Swiss Ephemeris and verified. Sixty-four scripts accompany this reading; <code>verify_audit.py</code> re-derives and asserts all 53 headline figures. Twenty-eight divisional schemes are computed and the sixteen Shodashavarga charts are printed in full. An earlier claim of node errors in D8 and D30 is retracted in §69; two unreconcilable columns are identified there and excluded. All fifty-five Brihat Parashara Hora Shastra chapters covered by the two supplied contents pages are assigned a state in §35, none left untouched; §36 measures what the forty-one chapters with no contents page would still decide, and §37 works the contents page for chapters 1–45 once it arrived — confirming the sixteen-varga apparatus against the source and withdrawing one earlier hedge. An interpretation within the framework of Jyotisha, presented on its own terms.
    </footer>
  </section>
</div>
'''
